...
func TestOpenclawClawdbotAlias(t *testing.T) {
	for _, alias := range []string{"clawdbot", "moltbot"} {
		t.Run(alias+" alias is not exposed by the termux fork", func(t *testing.T) {
			if _, ok := integrations[alias]; ok {
				t.Fatalf("%s should not be exposed in integrations", alias)
			}
		})

		t.Run(alias+" is not tracked as a selectable alias", func(t *testing.T) {
			if integrationAliases[alias] {
				t.Errorf("%s should not be present in integrationAliases", alias)
			}
		})
	}
}
